# Local Semantic Memory on a Small ARM VPS: How I Got OpenClaw Memory Search Working Without External APIs
 ...
 **March 19, 2026** | By Jingxiao Cai | **Updated June 23, 2026**
 ...
 **June 21 follow-up:** I made the active-memory canary rule even more explicit: long-budget success is a useful diagnostic ceiling, not a product-ready reply-path budget.
 ...
 **June 23 follow-up:** I tightened the canary interpretation: a long-budget active-memory canary can prove viability, but default reply-path authority still needs a separate latency, rollback, and trigger-scope gate.
...
## The Goal
 ...
 The target environment was not glamorous: a **small ARM VPS** running my personal OpenClaw deployment. In other words, exactly the kind of box where people assume local embeddings are going to be too slow, too fragile, or too annoying to be worth it.
 ...
 **OpenClaw local semantic memory absolutely worked on this small ARM VPS once the dependency and indexing strategy were corrected.**
...
- The boring stable remote landing zone ended up being `text-embedding-3-small` on a memory-only path, not the flashiest model I tried.
 ...
 That is why the remote stabilization story belongs in the separate follow-up, [Bigger Embeddings ≠ Better Memory](/jingxiao-cai-blog/text-embedding-3-small-openclaw-remote-memory.html), instead of being pasted wholesale into this post. This page is about proving that local semantic memory was viable on a modest self-hosted box. The later page is about what happened when I kept experimenting after that proof.
...
| --- | --- | --- | --- |
| **A** | `memory + sessions` | `perplexity/pplx-embed-v1-4b` | Isolate the effect of adding session transcripts while holding the stronger known-good model family constant. |
| **B** | `memory + sessions` | `perplexity/pplx-embed-v1-0.6b` | Test the cheaper lane on the same corpus instead of hiding a source-set change inside a model swap. |
 ...
 **Connection to skill governance:** the same anti-echo rule applies to agent skills. A skill radar should not create new skills merely because prior skill-radar prose kept mentioning them. See [Modernizing Agent Skills Without Growing a Skill Jungle](/jingxiao-cai-blog/modernizing-agent-skills-without-growing-skill-jungle.html) for the catalog-governance version of this lesson.
...
## May 2026 Follow-Up: Active Memory Canaries Need a Latency Budget
 ...
 A long-budget active-memory canary found a working setting in the narrow sense: the helper stopped immediately failing under the previous shorter timeout pattern, and early real runs produced non-empty summaries instead of timing out. But the observed pre-reply latency was still roughly **tens of seconds**.
...
| Layer | Observed shape | What it means |
| --- | --- | --- |
| **Embedding call** | Millisecond-level in direct probes. | The embedding endpoint was not the main bottleneck. |
| **Search wrapper** | Several seconds in ordinary memory-search probes. | Real cost, but not enough by itself to explain long pre-reply stalls. |
| **Active-memory helper** | Useful when it completed, but vulnerable to tail latency and host pressure. | The production question is the whole helper budget, not just embedding speed. |
 ...
 **Fast embeddings prove the retrieval substrate can work. They do not prove an embedded pre-reply helper is cheap enough for every turn.**
 ...
 The June 21 version is the blunt one: **long-budget success is a ceiling, not a comfort target.** It proves the active-memory path can survive when given enough room, which is valuable diagnostic evidence. It does not prove the path belongs in the synchronous pre-reply budget for ordinary chat turns.
 ...
 The June 23 version adds one more operational guardrail: even a successful long-budget active-memory canary should be recorded as **viability evidence**, not as default reply-path authority. Before making the helper synchronous by default, I would want a separate gate for trigger scope, timeout behavior, rollback, and end-to-end user-visible latency.
...
| Evidence | Good conclusion | Bad conclusion |
| --- | --- | --- |
| **Long-budget canary completes** | The path can work when given room. | The normal reply path should always wait for it. |
| **Recall is useful when returned** | The feature is worth preserving and tuning. | Every turn deserves a broad memory scan. |
| **Search substrate is fast** | The bottleneck is likely orchestration, query scope, or host pressure. | The end-to-end helper is automatically cheap. |
 ...
 **Related write-up:** I expanded the control-plane performance side of this into [The 10-Second Session List](/jingxiao-cai-blog/10-second-session-list-prefilter-row-build-agent-gateway.html).
...
- [Bigger Embeddings ≠ Better Memory: Why I Chose `text-embedding-3-small` for OpenClaw Remote Memory](/jingxiao-cai-blog/text-embedding-3-small-openclaw-remote-memory.html)
...
- [The Hidden Input Limit: When "202K Context" Doesn't Mean 202K](/jingxiao-cai-blog/hidden-input-limit-llm-apis.html)
